...
/**
 *	\file       mmiproject/mmiprojectindex.php
 *	\ingroup    mmiproject
 *	\brief      Home page of mmiproject top menu
 */
...
$langs->loadLangs(array("mmiproject@mmiproject"));
...
// Draft MyObject
if (! empty($conf->mmiproject->enabled) && $user->rights->mmiproject->read)
{
	$langs->load("orders");
...
if (! empty($conf->mmiproject->enabled) && $user->rights->mmiproject->read)
{
	...
	$sql.= " FROM ".MAIN_DB_PREFIX."mmiproject_myobject as s";
	//if (! $user->rights->societe->client->voir && ! $socid) $sql.= ", ".MAIN_DB_PREFIX."societe_commerciaux as sc";
