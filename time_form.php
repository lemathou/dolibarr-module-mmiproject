...
if ($addtimespent_multiple_userid) {
	$userids = GETPOST("userid", 'array:int');
}
...
//var_dump($date, $time); die();

// Months/users infos, with validated hours
$user_pay = [];
if (!empty($userids)) {
	$sql = 'SELECT * FROM '.MAIN_DB_PREFIX.'user_pay e
		WHERE e.fk_user IN ('.implode(', ', $userids).') AND e.date="'.$yearmonth.'-01"';
	//var_dump($sql); die();
	$q = $db->query($sql);
	if ($q) {
		if($r=$db->fetch_array($q)) {
			$user_pay[$r['fk_user']][$yearmonth] = $r;
		}
	}
}
//var_dump($user_pay); die();
		...
		if (!$error) {
			...
			if (empty($object->project->statut)) {
				...
				if (GETPOST("timehour") != '' && GETPOST("timehour") >= 0) {	// If hour was entered
					$object->timespent_date = dol_mktime(GETPOST("timehour", 'int'), GETPOST("timemin", 'int'), 0, $timemonth=GETPOST("timemonth", 'int'), GETPOST("timeday", 'int'), $timeyear=GETPOST("timeyear", 'int'));
					$object->timespent_withhour = 1;
				} else {
					$object->timespent_date = dol_mktime(12, 0, 0, $timemonth=GETPOST("timemonth", 'int'), GETPOST("timeday", 'int'), $timeyear=GETPOST("timeyear", 'int'));
				}
				foreach($userids as $userid) {
					// If user already validated the hours in month, we skipp
					if (!empty($user_pay[$userid][$yearmonth]['month_hours_sign_date'])) {
						setEventMessages($langs->trans('Month is already validated for user '.$users[$userid]['firstname'].', you cannot add time.'), null, 'errors');
						continue;
					}
...
foreach($userids as $userid) {
	if (!empty($user_pay[$userid][$yearmonth]['month_hours_sign_date'])) {
		$validated[] = $users[$userid]['firstname'];
	}
}
if (!empty($validated)) {
...
<?php } ?>
	...
	<tr>
		...
		<td rowspan="3"><input id="end_hour" name="end_hour" type="text" size="5" value="<?php echo isset($datefin) ?date('H:i') :'00:00'; ?>" style="text-align: right; border: 0;padding: 0;" /><br /><a href="javascript:;" onclick="$('#end_hour').val('<?php echo date('H:i'); ?>').change();" style="text-align: center;border: 1px solid gray;padding: 0 4px;border-radius: 5px;">Now</a></td>
